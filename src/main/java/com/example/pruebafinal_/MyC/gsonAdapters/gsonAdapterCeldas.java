...
//import com.example.pruebafinal_.intento1.Recurso;
...
//            }
//            celda.add("propiedades", jsonSerializationContext.serialize(celdas));
//
//            celda.addProperty("individuos", individuosJson.getAsString());
//        }
//
////        ListaEnlazada<Recurso> recursosListaEnlazada= celdas.getRecursos();
////        if (!recursosListaEnlazada.isVacia()){
////            JsonArray recursosJson= new JsonArray(recursosListaEnlazada.getNumeroElementos());
////            for (Integer i=0; i<recursosListaEnlazada.getNumeroElementos(); i++){
////                recursosJson.add(serialize(recursosListaEnlazada.getElemento(i).getData(), type, jsonSerializationContext));
////            }
////            celda.addProperty("individuos", individuosJson.getAsString());
////            celda.add("propiedades", jsonSerializationContext.serialize(celdas));
////        }
...
//    }
//
//    private JsonElement serialize(Individuo individuo, Type type2, JsonSerializationContext jsonSerializationContext) {
...
//                padresJson.add(serialize(padres.getElemento(i).getData(), type2, jsonSerializationContext));
//            }
...
////        Celdas celda= jsonDeserializationContext.deserialize(jsonCelda.getAsJsonObject(), Celdas.class); //no tenemos otras propiedades no?
...
//    }
//
//    public Individuo deserializeInd(JsonElement jsonElement, Type type2, JsonDeserializationContext jsonDeserializationContext) throws JsonParseException {
...
//                Individuo padre= deserializeInd(padres.get(i).getAsJsonObject(), type2, jsonDeserializationContext);
...
//        if (!colaIndividuo.isVacia()){
//            Cola<?> cola = new Cola<>();
//            cola.add(jsonDeserializationContext.deserialize(jsonElement, Cola.class));
